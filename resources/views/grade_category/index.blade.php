...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0">Grade Category</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
                  ...
                  <div class="card">
                    ...
                    <div class="card-body">
                      <table class="table table-bordered table-hover" id="table">
                        ...
                        <tbody>
                          @php $no = 1; @endphp
                          @foreach ($GradeCategory as $grade)
                          <tr>
                            <td>{{$no++}}</td>
                            <td>{{$grade->level}}</td>
                            <td>{{$grade->grade_name}}</td>
                            <td>
                                <a href="#" btn-id="{{$grade->id}}" title="Edit" class="btn btn-warning btn-edit"><i class="fas fa-edit"></i></a>
                                <a href="#" btn-id="{{$grade->id}}" title="Delete" class="btn btn-danger btn-delete"><i class="fas fa-trash"></i></a>
                            </td>
                          </tr>
                          @endforeach
                        </tbody>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h4 class="modal-title">Create New Grade Category</h4>
...
<div class="modal fade in" id="modalEdit" data-backdrop="static" data-keyboard="false">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <!-- Modal Header -->
      <div class="modal-header">
        <h4 class="modal-title">Edit Grade Category
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
        </button>
      </div>
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <span id="errorLevelEdit" class="text-red"></span>
        </div>
        <div class="form-group">
          <label for="grade_name">Name</label>
          <input type="text" name="grade_name" id="grade_name_update" value="{{old('grade_name')}}" class="form-control" required>
...
<div class="modal fade in" id="modalDelete" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <!-- Modal Header -->
        <div class="modal-header">
          <h4 class="modal-title">Delete Grade Category</h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
    ...
    $(document).ready(function(){
        ...
        $('.btn-add').click(function(){
            ...
            $('#form-create').submit(function(e){
                ...
                $.ajax({
                    url:"{{route('Grade_category.create')}}",
                    type:'POST',
                    data:formData,
                    processData: false,
                    contentType: false,
        ...
        $('.btn-edit').click(function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    $('#name_update').val(data.grade_name);
                    $('#form-edit').data('id',btnID);
                },
                error:function(response){
                    $('#errorLevelEdit').text(response.responseJSON.errors.level);
                    $('#errorNameEdit').text(response.responseJSON.errors.grade_name);
                }
            ...
            $('#form-edit').submit(function(e){
                ...
                var formData = new FormData(this);
                $.ajax({
                    url:"{{route('Grade_category.update')}}",
                    type:'POST',
                    data:{
                      id:ID,
                      level:$('#level_update').val(),
                      grade_name:$('#name_update').val()
                    },
                    beforeSend: function() {
                      modal_id.find('.modal-footer button').prop('disabled',true);
                      modal_id.find('.modal-header button').prop('disabled',true);
        ...
        $('.btn-delete').click(function(){
          ...
          $('#form-delete').submit(function(e){
                ...
                $.ajax({
                    url:"{{route('Grade_category.delete')}}",
                    type:'POST',
                    data:{
                      id:ID,
                    },
